...
public class KindergartenService {
	...
	public List<KindergartenInfo> getAllWithNonZeroCapacity() {
		...
		return kindergartens.stream()
				.map(garten -> new KindergartenInfo(garten.getId(), garten.getName(), garten.getAddress(),
						garten.getElderate(), garten.getDirector(), garten.getCapacityAgeGroup2to3(),
						garten.getCapacityAgeGroup3to6(), garten.getPlacesTakenAgeGroup2to3(),
						garten.getPlacesTakenAgeGroup3to6()))
				.collect(Collectors.toList());
	}

	/**
	 * Get all kindergarten ID's, names and addresses where capacity in both age
	 * groups is zero
	 *
	 * @return a list of kindergarten ID, names and addresses sorted by name ASC
	 */
	...
	public List<KindergartenInfo> getAllWithZeroCapacity() {
		...
		List<Kindergarten> kindergartens = gartenDao.findAllWithZeroCapacity(Sort.by("name").ascending());
	 ...
	 */
	@Transactional(readOnly = true)
	public Page<KindergartenDTO> getKindergartenPage(Pageable pageable, String search) {
		if (search == null) {
			return gartenDao.findAllKindergarten(pageable).map(KindergartenDTO::from);
		}
		return gartenDao.findByNameContainingIgnoreCase(search, pageable).map(KindergartenDTO::from);
	}
	...
	public void createNewKindergarten(KindergartenDTO kindergarten) {
		...
		gartenDao.save(new Kindergarten(kindergarten.getId(), kindergarten.getName(), kindergarten.getAddress(),
				kindergarten.getDirector(), kindergarten.getElderate(), kindergarten.getCapacityAgeGroup2to3(),
	...
	public void updateKindergarten(String id, KindergartenDTO updatedInfo) {
		...
		current.setDirector(updatedInfo.getDirector());
	 ...
	 */
	@Transactional(readOnly = true)
	public Page<KindergartenDTO> getKindergartenPageByNameAndAddress(Pageable pageable, String search) {

		return gartenDao.findByNameAndAddressContainingIgnoreCase(search, pageable).map(KindergartenDTO::from);
	}
