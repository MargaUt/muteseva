...
public interface KindergartenDAO extends JpaRepository<Kindergarten, String> {
	...
	@Query("SELECT new Kindergarten(k.id, k.name , k.address, k.director, k.elderate, k.capacityAgeGroup2to3, k.capacityAgeGroup3to6) FROM Kindergarten k WHERE LOWER(k.name) LIKE LOWER(concat('%', ?1,'%'))")
	Page<Kindergarten> findByNameContainingIgnoreCase(String name, Pageable pageable);

	@Query("SELECT new Kindergarten(k.id, k.name , k.address, k.director, k.elderate, k.capacityAgeGroup2to3, k.capacityAgeGroup3to6) FROM Kindergarten k")
	...
	@Query("SELECT k FROM Kindergarten k WHERE k.capacityAgeGroup2to3=0 AND k.capacityAgeGroup3to6=0")
	List<Kindergarten> findAllWithZeroCapacity(Sort ascending);
	...
	@Query("SELECT new Kindergarten(k.id, k.name , k.address, k.director, k.elderate, k.capacityAgeGroup2to3, k.capacityAgeGroup3to6) FROM Kindergarten k WHERE LOWER(k.name||' '||k.address) LIKE LOWER(concat('%', ?1,'%'))")
	Page<Kindergarten> findByNameAndAddressContainingIgnoreCase(String name, Pageable pageable);
