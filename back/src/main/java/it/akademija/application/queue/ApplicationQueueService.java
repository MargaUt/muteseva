...
public class ApplicationQueueService {
	...
	public ResponseEntity<String> confirmApplicationsInQueue() {
		...
		if (applicationDao.findNumberOfUnprocessedApplications() > 0) {
			...
			return new ResponseEntity<>("Nepavyko patvirtinti eilės. Pradžioje eilė turi būti suformuota.",
		...
		return new ResponseEntity<>("Eilė patvirtinta", HttpStatus.OK);
	...
	public Page<ApplicationQueueInfo> getApplicationQueueInformation(Pageable pageable, String search) {
                if (search==null) {
		    return applicationDao.findQueuedApplications(pageable);
                }
                return applicationDao.findQueuedApplicationsContaining(search, pageable);
	}
