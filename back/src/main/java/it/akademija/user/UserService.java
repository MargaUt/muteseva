...
public class UserService implements UserDetailsService {
	 ...
	 * password or username can not be updated. Using for Compensations
	 *
	 ...
	 */
	@Transactional
	public User updateUserDataInfo(UserInfo userData, String username) {
	...
	public Page<UserInfo> getAllUsers(Pageable pageable) {
		...
		Page<UserInfo> dtoPage = users.map(new Function<User, UserInfo>() {
			...
			public UserInfo apply(User user) {
				UserInfo dto = new UserInfo(user.getRole(), user.getUsername());
				return dto;
			}
	...
	public UserInfo getUserDetails(String username) {
		...
		if (user.getRole().equals(Role.USER)) {
			return new UserInfo(user.getRole(), user.getName(), user.getSurname(),
		...
		return new UserInfo(user.getRole(), user.getName(), user.getSurname(), user.getUsername(), user.getEmail());
